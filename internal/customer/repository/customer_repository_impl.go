...
func (repo *CustomerRepositoryImpl) GetBalanceWithTx(ctx context.Context, tx *sql.Tx, customerID int64) (*model.TCustomers, error) {
...
func (repo *CustomerRepositoryImpl) UpdateBalanceWithTx(ctx context.Context, tx *sql.Tx, customerID int64, balance float64) error {
	...
	_, err := tx.ExecContext(ctx, queryUpdateBalanceTx, balance, customerID)
	return err
}
