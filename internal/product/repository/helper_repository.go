...
func rowsToProduct(rows *sql.Rows) (*model.TProduct, error) {
	...
	err := rows.Scan(&product.ID, &product.SellerID, &product.ProductName, &product.Price, &product.Stock, &product.CategoryID, &base.Created, &base.CreatedBy, &base.Updated, &base.UpdatedBy)
	if err != nil {
		return nil, err
	}
...
func scanProducts(rows *sql.Rows) ([]*model.TProduct, error) {
	...
	for rows.Next() {
		...
		err := rows.Scan(
			&product.ID, &product.ProductName, &product.SellerID, &product.Price, &product.Stock, &product.CategoryID,
