...
func (repo *TransactionRepositoryImpl) CreateWithTx(ctx context.Context, tx *sql.Tx, data model.TTransaction) (*model.TTransaction, error) {
	...
	err = statement.QueryRowContext(ctx, data.CustomerID, data.CartID, data.TotalAmount, data.TransactionDate, data.Status, data.Base.Created, data.Base.CreatedBy).Scan(&id)
	if err != nil {
		return nil, err
	}
	return &data, nil
